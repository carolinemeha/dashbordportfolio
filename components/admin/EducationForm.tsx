...
import { GraduationCap, Building, Calendar, BookOpen, X, Plus } from 'lucide-react';
      ...
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto overflow-x-hidden glass-card border-border/50 text-foreground">
        <DialogHeader className="pb-4 border-b border-border/40">
          ...
          <DialogDescription className="text-muted-foreground">
            {education ? 'Mettez à jour les détails de ce diplôme ou certification.' : 'Ajoutez un nouveau diplôme ou une formation à votre parcours.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="institution" className="text-foreground flex items-center gap-2">
                <Building className="h-4 w-4 text-muted-foreground" /> Établissement <span className="text-destructive">*</span>
              </Label>
              <Input
                id="institution"
          ...
          <div className="space-y-3 p-4 bg-secondary/10 rounded-xl border border-border/30">
            <Label className="text-foreground flex items-center gap-2">
              <BookOpen className="h-4 w-4 text-muted-foreground" /> Cours / Matières suivies
            </Label>
            ...
            <div className="flex flex-wrap gap-2 pt-2 min-h-[32px]">
              <AnimatePresence>
                ...
                {formData.courses.length === 0 && (
                  <span className="text-sm text-muted-foreground italic flex items-center">Aucun cours ajouté.</span>
                )}
              </AnimatePresence>
            </div>
          </div>
